...
Browser
  ...
  -> Google Ads API
  -> Search Console API
  -> Altegio aggregate reporting API (после выдачи read-only прав)
```
...
- Altegio подключается отдельным read-only user token; production-секреты
  booking-проекта не монтируются в analytics container.
...
Altegio должен предоставлять только агрегированные операционные показатели без
имён, телефонов и email. После стабилизации интеграции дневные агрегаты следует
сохранять локально, чтобы строить исторические сравнения независимо от
ограничений upstream API.
